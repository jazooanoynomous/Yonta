...
const StatsCard = ({ title, total, data }) => {
  ...
  return (
    <div className="bg-backgroundgray p-4 rounded-lg shadow-sm" style={{ width: '198px', height: '230px' }}>
      <h2 className="text-[14px] font-bold text-bluetext mb-2">{title}</h2>
      <p className="text-[32px] font-bold text-bluetext mb-4">{total.toLocaleString()}</p>
      ...
      <div className="mt-4 overflow-y-auto" style={{ maxHeight: '120px' }}>
        {data.map((item, index) => (
          <div key={index} className="flex space-x-1 text-[10px] text-blacktext mb-1">
